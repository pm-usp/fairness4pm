...
class MetaFairClassifier(Transformer):
    ...
    def predict_proba(self, dataset):
        """Pre-process and return probabilities.

        Args:
            dataset (BinaryLabelDataset): Dataset containing features to be
                classified.

        Returns:
            numpy.ndarray: Probability estimates for all classes. The columns
                must be ordered according to the values in ``self.classes_``.
        """
        if self.model is None:
            raise RuntimeError("Model needs to be fit first.")

        # O modelo interno (t) retorna um score.
        # A implementação original de predict() converte isso para uma "probabilidade"
        # com a fórmula (t + 1) / 2. Faremos o mesmo.
        t = self.model(dataset.features)
        scores_favorable = ((t + 1) / 2).ravel()

        # Retorna um array com shape (n_samples, 2) com [prob_classe_0, prob_classe_1]
        proba = np.vstack([1 - scores_favorable, scores_favorable]).T
