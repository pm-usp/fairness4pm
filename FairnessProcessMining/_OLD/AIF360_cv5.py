...
def InAdversarialDebiasingOptuna(X_train, y_train, X_test, y_test, 
    ...
    def objective(trial):
        # Hyperparameter search space
        #Event Logs = Hospital, Lending and Renting
        #num_epochs = trial.suggest_categorical('num_epochs', [50, 100]) 
        #units = trial.suggest_categorical('classifier_num_hidden_units', [50, 100])
        #weight = trial.suggest_float('adversary_loss_weight', 0.05, 1.2, step=0.05)

        #Event Logs = Hiring
        num_epochs = trial.suggest_categorical('num_epochs', [25, 50, 100, 150]) 
        units = trial.suggest_categorical('classifier_num_hidden_units', [25, 50, 100])
        weight = trial.suggest_float('adversary_loss_weight', 0.1, 0.8, step=0.05)
        
        #weight = trial.suggest_float('adversary_loss_weight', 0.05, 0.5, step=0.02) -- Não ficou bom
        #weight = trial.suggest_float('adversary_loss_weight', 1e-2, 0.8, log=True) -- Esse ficou ok, apenas lending ficou acima.
        #weight = trial.suggest_float('adversary_loss_weight', 1e-2, 0.7, log=True) -- Não ficou bom
        #weight = trial.suggest_float('adversary_loss_weight', 1e-2, 1.2, log=True)
