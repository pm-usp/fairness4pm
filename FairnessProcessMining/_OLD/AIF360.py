...
from AIF360Adapted.preprocessing.Reweighing import Reweighing
from AIF360Adapted.preprocessing.DisparateImpactRemover import DisparateImpactRemover
from AIF360Adapted.preprocessing.LFR import LFR
from AIF360Adapted.inprocessing.AdversarialDebiasing import AdversarialDebiasing
from AIF360Adapted.inprocessing.PrejudiceRemover import PrejudiceRemover
from AIF360Adapted.postprocessing.EqOddsPostprocessing import EqOddsPostprocessing
from AIF360Adapted.postprocessing.CalibratedEqOddsPostprocessing import CalibratedEqOddsPostprocessing
...
def PreLFRRandomForestOptuna(X_train, y_train, X_test, y_test, 
    ...
    """
    Applies the Learning Fair Representations (LFR) pre-processing technique and then
    trains and optimizes a RandomForestClassifier on the transformed data.
    Hyperparameters for both LFR and RandomForest are optimized using Optuna.
    """
    
    original_attributes = X_train.columns.tolist()
    label_col_name = y_train.name if y_train.name is not None else 'Target'

    # LIME explainer é inicializado com os dados ORIGINAIS, pois queremos explicações
    # em termos das features originais. A função de predição lidará com a transformação.
    ...
    # Dataset AIF360 do treino completo, usado no fit do LFR final
    dataset_train_orig = prepare_aif360_data(X_train, y_train, protected_attribute)

    def objective(trial):
        # --- Hiperparâmetros para LFR ---
        # k: dimensionalidade da nova representação
        lfr_k = trial.suggest_int('lfr_k', 5, 20)
        # Pesos para a função de perda do LFR
        lfr_Ax = trial.suggest_float('lfr_Ax', 0.01, 1.0, log=True) # Reconstrução
        lfr_Ay = trial.suggest_float('lfr_Ay', 0.01, 1.0, log=True) # Acurácia
        lfr_Az = trial.suggest_float('lfr_Az', 0.01, 1.0, log=True) # Fairness
        
        # --- Hiperparâmetros para RandomForest ---
        # (código para sugerir n_estimators, max_depth, etc. como antes)
        n_estimators = trial.suggest_int('n_estimators', 50, 200)
        ...
        skf = StratifiedKFold(n_splits=3, shuffle=True, random_state=SEED) # Usando k=3 para velocidade
        f1_scores = []
        di_scores = []
        ...
        for train_index, val_index in skf.split(X_train, y_train):
            ...
            # 1. Treinar LFR no fold de treino
            ...
            # 2. Transformar os dados de treino e validação do fold
            ...
            # 3. Treinar RandomForest nos dados transformados
            ...
            # Para o DI, usamos as features originais (X_fold_val_orig) para contexto,
            # mas as predições feitas nos dados transformados.
    ...
    print("\n--- Gerando Visualizações do Estudo Optuna (PreLFR) ---")
    ...
    if best_trial_found:
        ...
        # Separar parâmetros do LFR e do RandomForest
        ...
        print("\n--- Configuração Final Selecionada (Baseada na CV) ---")
        if best_score_cv is not None: print(f"F1-score (CV): {best_score_cv:.4f}")
        if final_di_cv is not None: print(f"Disparate Impact (DI) (CV): {final_di_cv:.4f}")
        print(f"Melhores Hiperparâmetros (LFR + RF): {final_best_params}")
        
        # --- Treinamento Final ---
        # 1. Treinar LFR final no conjunto de treino completo
        ...
        # 2. Transformar treino e teste com o LFR treinado
        ...
        # 3. Treinar RandomForest final nos dados transformados
        ...
        # 5. LIME para explicar o sistema combinado (LFR + RF)
        def lfr_rf_predict_proba(X_original_np):
            # Função para LIME que aplica a pipeline completa em dados no formato original
            X_orig_df = pd.DataFrame(X_original_np, columns=original_attributes)
            # Dummy 'y' para criar o dataset AIF360
        ...
        return df_final, best_model, final_best_params, best_score_cv, explanations
    else:
...
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
"""                 In-processing Models: Prejudice Remover                  """
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""


class PrejudiceRemoverWrapper(BaseEstimator, ClassifierMixin):
    """
    Wrapper para o PrejudiceRemover da AIF360 para torná-lo compatível
    com a interface scikit-learn e para uso com LIME.
    """
    def __init__(self, sensitive_attr_name, class_attr_name='Target', eta=1.0, seed=None):
        ...
        self.class_attr_name = class_attr_name # Nome da coluna label no dataset AIF360
        self.eta = eta
        self.seed = seed # Embora PrejudiceRemover não use seed diretamente, guardamos por consistência
        
        self.model_ = None
        self.columns_ = None
        self.classes_ = None
        self.scaler_ = None # Para escalar features, pois PR pode ser sensível

    def _set_seeds_if_needed(self):
        if self.seed is not None:
            np.random.seed(self.seed)
            # tf.compat.v1.set_random_seed(self.seed) # Não necessário se PR não usa TF
    ...
    def fit(self, X, y):
        ...
        # PrejudiceRemover pode ser sensível à escala das features
        self.scaler_ = MinMaxScaler()
        X_scaled = self.scaler_.fit_transform(X)
        X_scaled_df = pd.DataFrame(X_scaled, columns=self.columns_, index=X.index)

        # y já é uma Series, prepare_aif360_data espera X e y
        # y.name deve ser o nome da coluna label (ex: 'Target')
        y_named = y.copy()
        if y_named.name is None:
            y_named.name = self.class_attr_name # Garante que y tenha o nome esperado
        ...
        self.model_ = PrejudiceRemover(eta=self.eta, 
                                       ...
                                       class_attr=self.class_attr_name) # class_attr é o nome da label em aif360_dataset
    ...
    def predict(self, X):
        ...
        # Verifica se a entrada é um DataFrame para preservar o índice original
        # Se for um array NumPy (do LIME), o índice será None (padrão do pandas)
        original_index = X.index if isinstance(X, pd.DataFrame) else None
        
        X_scaled = self.scaler_.transform(X)
        # Usa o índice original (se existir) e as colunas salvas durante o 'fit'
    ...
    def predict_proba(self, X):
        ...
        pred_results = self.model_.predict(aif360_dataset_pred)
        scores = pred_results.scores.ravel()
        proba_favorable = 1 / (1 + np.exp(-scores)) # Aplica sigmoide
        
        proba = np.vstack([1 - proba_favorable, proba_favorable]).T
        
        # Verificação (opcional, mas bom ter)
        if not np.allclose(proba.sum(axis=1), 1.0):
            print(f"Aviso: Probabilidades em PrejudiceRemoverWrapper não somam 1. "
                  f"Primeiras 5 somas: {proba.sum(axis=1)[:5]}. Normalizando...")
            # Normaliza se não somar 1 (pode acontecer por precisão numérica após sigmoide)
            proba_sum = proba.sum(axis=1, keepdims=True)
            proba_sum[proba_sum == 0] = 1 # Evita divisão por zero se ambas as probas forem 0
            proba = proba / proba_sum
...
def InPrejudiceRemoverOptuna(X_train, y_train, X_test, y_test, 
                             ...
                             protected_attribute, num_trials=50):
    """
    Implements the PrejudiceRemover model with AIF360.
    ...
    def objective(trial):
        # Hiperparâmetro para PrejudiceRemover
        ...
        for train_index, val_index in skf.split(X_train, y_train):
            ...
            # O wrapper lida com a preparação dos dados internamente
            model.fit(X_fold_train, y_fold_train_named) 
            y_pred_val = model.predict(X_fold_val)

            # --- INÍCIO DA CORREÇÃO ---
            # Verifica se o modelo retornou um array de predições vazio.
            if y_pred_val.shape[0] == 0:
                # print(f"AVISO - Trial {trial.number}, Fold: PrejudiceRemover retornou predições vazias. Atribuindo F1=0, DI=0.") # Comentado
                f1_scores.append(0.0)
                di_scores.append(0.0)
                continue  # PULA PARA O PRÓXIMO FOLD, evitando o erro
            # --- FIM DA CORREÇÃO ---

            y_fold_val_named = y_fold_val.copy() # Recria para garantir o alinhamento
            if y_fold_val_named.name is None: y_fold_val_named.name = label_col_name
            if not y_fold_val_named.index.equals(X_fold_val.index):
                 y_fold_val_named = pd.Series(y_fold_val_named.values, index=X_fold_val.index, name=y_fold_val_named.name)
    ...
    # --- SEÇÃO DE VISUALIZAÇÃO DO OPTUNA ---
    print("\n--- Gerando Visualizações do Estudo Optuna (InPrejudiceRemover) ---")
    if len(study.trials) > 0:
        try:
            fig_pareto = optuna.visualization.plot_pareto_front(study, target_names=["F1-score Médio CV", "Fairness Score Médio CV"])
            fig_pareto.show()
            ...
            # Você pode descomentar os outros plots se desejar
    ...
    if best_trial_found:
        ...
        print(f"Melhores Hiperparâmetros (PrejudiceRemover): {final_best_params_to_return}")
        ...
        # PrejudiceRemover (versão baseada em subprocess) não tem sessão TF para fechar.
        # Se o seu wrapper tem, a chamada estaria aqui.
        # if hasattr(best_model, 'close_session'):
        #     best_model.close_session()
